...
export class ListReportsComponent implements OnInit {
  ...
  MyDataSource: any;
  displayedColumns: string[] = ['report_id',
                                'report_type', 
                                'created_on', 
                                'created_by', 
                                'report_status', 
  ...
  getReportsData() {
    ...
    this.sortQuery = (this.sortQuery ? this.sortQuery : '&sort_order=desc&sort_by=created_on');
    query += this.sortQuery;   
    this.subscription = this.reportService.getListOfReports(query).subscribe(res => {
  ...
  getReportById(successMsg, errorMsg) {
    ...
    this.reportService.getReportById(this.selectedReportID).subscribe(res=>{
      ...
      this.reportService.downloadFile(reportContents, headerList, 'Report_'+this.selectedReportType+'_'+this.selectedReportID, this.selectedFileFormat);
  ...
  sortData(ev) {
    ...
    this.sortQuery = '&sort_order=' + ev.direction.toLowerCase() + '&sort_by=' + ev.active.toLowerCase();;
    this.getReportsData();
  }
