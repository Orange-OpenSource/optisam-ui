...
// Get metadata
  getAttrMetadata() {
    this.configurationService.listConfiguration(this.eqTypeForUpdate).subscribe((response: any) => {
      if(response.configurations) {
        response.configurations.filter(res=>res.config_id == this.configID).map((res)=>{this.eqTypeMetadata =res;});
    ...
    this.attributeList.map((item) => {
      ...
      // for(var i = 0; i < this.eqTypeMetadata.length; i++) { // get creation data
        for(var j = 0; j< this.eqTypeMetadata.config_attributes.length; j++) {
          if(this.eqTypeMetadata.config_attributes[j].attribute_name === item.name)  {
            attrFound = true;
            attrID = this.eqTypeMetadata.config_attributes[j].attribute_id;
            attrFileName = this.eqTypeMetadata.config_attributes[j].config_filename;
            break;
          }
        }
      // }
  ...
  checkIfAnyChangeMade() {
    ...
    const files = this.filesArray.value.filter((val)=>{if(val.name && val.name != ''){return val}});
    const allFiles = this.configObj.attributes.filter((res)=>{if(res.fileName && (res.fileInvalid ||(res.file && res.file.length))) {return res}});
    if(allFiles && allFiles.length > 0) {this.disableResetFlag = false;}
    else {this.disableResetFlag = true;}
    if(files && files.length>0) {this.NoFileSelectedFlag = false;}
    if(this.deletedConfigAttrs.length>0) {this.NoChangesFlag = false;this.NoFileSelectedFlag = false; this.disableResetFlag = false;}
    if(files && files.length==0 && this.deletedConfigAttrs.length === this.eqTypeMetadata.config_attributes.length) { this.NoFileSelectedFlag = true;}
  }
  ...
  deleteAttributeById(index) {
    ...
    // for(var i = 0; i < this.eqTypeMetadata.length; i++) { // get creation data
      for(var j=0; j< this.eqTypeMetadata.config_attributes.length; j++)
      {
        if (this.eqTypeMetadata.config_attributes[j].attribute_name === this.configObj.attributes[index].attributeName) {
          attrFound = true;
          break;
      }
      }
  //  }  
   if(attrFound) {
    if(this.deletedConfigAttrs.indexOf(this.configObj.attributes[index].attributeID) === -1)
    {this.deletedConfigAttrs.push(this.configObj.attributes[index].attributeID);}
   }
