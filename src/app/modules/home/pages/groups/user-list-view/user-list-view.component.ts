...
import { DefaultResponse, ResendActivationLinkParams, UserStatus } from '@core/modals';
...
export class UserListViewComponent implements OnInit {
  ...
  get activeUserStatus(): UserStatus {
    return UserStatus.active;
  }

  get inactiveUserStatus(): UserStatus {
  ...
  loadData() {
    ...
    if ((this.role === 'ADMIN') || (this.role === 'SUPER_ADMIN')) {
      this.displayedColumns = ['last_name', 'first_name', 'user_id', 'groups', 'role', 'status', 'action'];
  ...
  resendActivationLink(user: any): void {
    if (user.account_status === this.activeUserStatus || (user.isSendingLink || false)) return;
    user.isSendingLink = true;

    const params: ResendActivationLinkParams = {
      user: user.user_id
    }
    this.as.resendActivationLink(params).subscribe((res: DefaultResponse) => {
      user.isSendingLink = false;
      if (res.success) {
        this.ss.commonPopup({
          title: "SUCCESS_TITLE",
          message: "ACTIVATION_LINK_SENT",
          singleButton: true,
          buttonText: "OK",
          messageVariable: { email: user?.user_id }
      ...
      this.ss.commonPopup({
        title: "ERROR_TITLE",
        message: e.message || "ACTIVATION_LINK_SENT_ERROR",
        singleButton: true,
        buttonText: "OK"
